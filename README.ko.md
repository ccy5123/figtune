...
둘째 줄은 창을 띄우므로 `[gui]`가 필요하다. 그것이 Qt다. 그냥
`pip install figtune`은 650MB 작지만 `render`, `refresh`, `merge`,
`normalize` 서브커맨드만 남는다 — 빌드 서버에서는 쓸모가 있고 데스크톱에서는
아니다.
...
│   ├── drag.py        # 끌기 → 프로퍼티 값
│   ├── snap.py        # 붙이기 기하 (순수 함수)
│   ├── palette.py     # matplotlib이 이름을 가진 색
