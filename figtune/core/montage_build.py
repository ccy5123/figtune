...
Draws the plot(ax) function of each panel script into a subplot grid. The
result is an ordinary matplotlib Figure, so figtune can open and edit it as a
whole, and this file also runs on its own without figtune.

To add or replace panels, edit PANELS below.
"""
...
for _ax in _axes[len(PANELS):]:          # leave the spare cells empty
