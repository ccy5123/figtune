...
Draws the plot(ax) function of each panel script into a grid. The result is an
ordinary matplotlib Figure, so figtune can open and edit it as a whole, and
this file also runs on its own without figtune.

CELLS gives each panel its place as (row, col, rowspan, colspan), so a panel
may span several grid cells. To add, move or replace panels, edit the lists
below.
"""
...
# (row, col, rowspan, colspan) — same order as PANELS
CELLS = {cells!r}
...
def generate_subplot_script(mspec: MontageSpec, out_path: Path | str,
    ...
    out.write_text(MERGE_TEMPLATE.format(
        panels=entries, cells=mspec.placements(), labels=labels,
