...
def detect_plot_function_source(source: str) -> str | None:
    """detect_plot_function의 소스 문자열 판. 파일을 두 번 읽지 않는다."""
    try:
...
def panel_problem(script: Path | str) -> str | None:
    ...
    try:
        block = panel_block(source, path.name, "_probe")
    except (inline.InlineError, autowrap.WrapError) as exc:
        return str(exc)
    return None if block else _t("ax를 받는 plot(ax) 함수가 없습니다")
